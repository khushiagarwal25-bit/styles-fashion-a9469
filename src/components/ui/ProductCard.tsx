        ...
        >
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {product.is_new_arrival && (
              <span className="bg-stone-900 text-white text-[10px] px-2 py-1 font-medium tracking-widest uppercase">
                New
              </span>
            )}
            {discount && (
              <span className="bg-red-500 text-white text-[10px] px-2 py-1 font-medium tracking-wider">
                -{discount}%
              </span>
            )}
            {product.is_trending && (
              <span className="bg-amber-500 text-white text-[10px] px-2 py-1 font-medium tracking-widest uppercase">
                Trending
              </span>
            )}
          </div>
          ...
          >
            <Heart
              size={15}
              className={wishlisted ? "fill-red-500 text-red-500" : "text-stone-600"}
            />
          </button>
          ...
          {product.sizes && product.sizes.length > 0 && (
            <div className="flex gap-1 mt-2 flex-wrap">
              {product.sizes.slice(0, 4).map((size) => (
                <span
                  key={size}
                  className="text-[10px] border border-stone-200 px-1.5 py-0.5 text-stone-400"
                >
                  {size}
                </span>
              ))}
